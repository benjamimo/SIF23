...
# ╔═╡ 7d5c7154-f38c-11ed-3684-4319a853c51e
begin
...
# ╔═╡ 0ee9721d-b0aa-48a0-b314-f16e62366bee
html"""<style>
main {
    max-width: 1050px;
}
...
# ╔═╡ 2780b44f-f718-41e6-8538-91dba263df3e
TableOfContents()

# ╔═╡ 96279818-cd13-4daf-973d-355133d564c6
md"""
# Magnetism

In this notebook we will explore some textbook examples from electricity and magnetism.  First, we will load the packages.  In particular we will use Plots, DifferentialEquations and QuadGK.

"""

# ╔═╡ 27a4d22e-ff84-4762-90a0-d55c8cfccb13
md"""
## Circular loop

The magnetic field of a circular loop can be expressed as

$\vec{B}(\vec{r}) = \frac{\mu_0 I}{4\pi}\int_C \frac{d\vec{s}\times \hat{r}}{r^2},$
where $\mu_0$ is the permeability of free space, $I$ the current, $C$ is the curve corresponding to the loop and $\vec{r}$ is the position vector.
"""

# ╔═╡ d5ceeecc-aceb-4f79-9a65-0e4c83363c0e
begin
	function LoopBx(x,y,z,K,R,t0,tf)
	    dBx(x,y,z,t) = K*R*z*cos(t)/((x-R*cos(t))^2 + (y-R*sin(t))^2 + (z)^2)^(3/2)
	    Bx, errBx = quadgk(t -> dBx(x,y,z,t), t0, tf, rtol=1e-16)
	    
	    return Bx
	end
	
	function LoopBy(x,y,z,K,R,t0,tf)
	    dBy(x,y,z,t) = K*R*z*sin(t)/((x-R*cos(t))^2 + (y-R*sin(t))^2 + (z)^2)^(3/2) 
	    By, errBy = quadgk(t -> dBy(x,y,z,t), t0, tf, rtol=1e-16)
	    
	    return By
	end
	
	function LoopBz(x,y,z,K,R,t0,tf)
	    dBz(x,y,z,t) = K*(R^2 - x*cos(t) - y*sin(t))/((x-R*cos(t))^2 + (y-R*sin(t))^2 + (z)^2)^(3/2)
	    Bz, errBz = quadgk(t -> dBz(x,y,z,t), t0, tf, rtol=1e-16)
	    
	    return Bz
	end
end

# ╔═╡ 8860ee90-ea17-4584-80c1-8490fd6653f4
md"""
### Visualization

To achieve the visualization of $\vec{B}(\vec{r})$, we numerically computed the integral corresponding to the magnetic field in each point $(x,y,z)$.
"""

# ╔═╡ f152b4d0-e336-446b-a6ef-724d4ea7497f
begin
	# Size of numerical window in terms of circle size
	xmax = 2.5
	points = 8
	
	# Generates ranges for xs, ys and zs
	...
	zs = xmax*(2/points)*collect(range(-points/2,length=points,stop=points/2-1))
	
	# Coordinates ... is it correct?
	mx, ny, pz = length(xs), length(ys), length(zs)
	Xs = repeat(xs, 1, ny, pz)
	Ys = repeat(ys', mx, 1, pz)
	Zs = repeat(reshape(zs, 1, 1, pz), mx, ny ,1);
end;

# ╔═╡ faba2857-8643-409c-b7d9-e9e7458ba6ae
begin
	K1 = 1.0 # Constant
	R1 = 1.0 # Radius of the loop
	Bx = LoopBx.(Xs,Ys,Zs,K1,R1,0.0,2*pi)
	By = LoopBy.(Xs,Ys,Zs,K1,R1,0.0,2*pi)
	Bz = LoopBz.(Xs,Ys,Zs,K1,R1,0.0,2*pi)
end;

# ╔═╡ 51161e0d-5d39-4241-be61-b59a48dcde1b
normB = sqrt.(Bx.^2 .+ By.^2 .+ Bz.^2);

# ╔═╡ bb8f1ca8-5ff2-4d00-be4f-34e7fde0f5b8
begin
	fac = 5.0
	tt = 0:0.01:2pi
	p1 = plot3d(proj_type=:persp, bg=RGB(1,1,1))
	p1 = plot3d!(R1*cos.(tt), R1*sin.(tt), 0.0*cos.(tt), color=:red, linewidth=0.5, linestyle= :solid)
	p1 = carcaj(Xs, Ys, Zs, fac*Bx, fac*By, fac*Bz; samppx=2^2, samppy=2^2, samppz=2^2)
	
	# Draw axis to overcome weird scaling behaviour
	winsize = 0.9*maximum([maximum(abs,Xs),maximum(abs,Ys),maximum(abs,Zs)])
	ejes(winsize;cl=:black)
end;

# ╔═╡ 157f55dc-37f3-4de0-bb3c-0d5cd591ce12
md"""
anglec1 $(@bind ancA Slider(0.0:1.0:360, default=45.0, show_value=true))
anglec2 $(@bind ancP Slider(0.0:1.0:360, default=45.0, show_value=true))
"""

# ╔═╡ 8114d55d-b37d-4d97-8b0a-ddc970cdcef0
plot(p1, camera=(ancA,ancP), size=(1920,1920))

# ╔═╡ 21fecd70-c199-434a-92f1-4de0246dde44
md"""
## Infinite wire
"""

# ╔═╡ 93224c6e-f167-4669-a8a1-5adc7d6a9a23
md"""
The magnetic field for an infinite long wire of radius $R$ can be written as

$\vec{B}(\vec{r}) = \frac{\mu_0 I}{2\pi r}\hat{\theta},$
for $r\ge R$.
"""

# ╔═╡ a7dcd4fe-6902-4171-9212-a50efc03c635
begin
	function wireBx(x,y,z,mu0,I,R)
	    Bx = (-sin(atan(y,x)))*(mu0*I*(x^2+y^2)/(2*pi*R^2))*(x^2+y^2<R^2) + 
			 (-sin(atan(y,x)))*(mu0*I/(2*pi*sqrt(x^2+y^2)))*(x^2+y^2 >= R^2)
	end
	   
	function wireBy(x,y,z,mu0,I,R)
	    By = (cos(atan(y,x)))*(mu0*I*(x^2+y^2)/(2*pi*R^2))*(x^2+y^2<R^2) + 
			 (cos(atan(y,x)))*(mu0*I/(2*pi*sqrt(x^2+y^2)))*(x^2+y^2 >= R^2)
	end
end

# ╔═╡ 57c7754e-8e6e-4123-8485-b84587ad73b2
md"""
The idea is to throw a charged particle and find its trajectory.  To do that, we numerically solved the following equation

$q\left(\frac{d\vec{r}}{dt}\times\vec{B}\right) = m\frac{d^2\vec{r}}{dt^2}.$
"""

# ╔═╡ f057c8a0-a181-4228-a9d1-11a003667192
function magneticsystem!(du, u, p, t)
    q,m,R,I,mu0 = p
    ...
    # I = 1.0
    # mu0 = 1.0
    Bx = wireBx(x,y,z,mu0,I,R)
    By = wireBy(x,y,z,mu0,I,R)
    Bz = 0.0

    du[1] = dx
    du[2] = (q/m)*(dy*Bz - dz*By)
    du[3] = dy
    du[4] = (q/m)*(dz*Bx - dx*Bz)
    du[5] = dz
    du[6] = (q/m)*(dx*By - dy*Bx)
end

# ╔═╡ 791f686a-ba67-438d-a8b7-68cf29294735
md"""
We define the initial conditions $\vec{u_0} = \{x, v_x, y, v_y, z, v_z \}$, and the temporal window $t_{\text{span}}$.
"""

# ╔═╡ 953b1236-8575-4bb6-b3a2-c73b0bdb7d62
begin
	u0 = [1.0; 0.05; 0.0; 0.025; -3.0; 0.05]
	tspan = (0.0, 225.0)
end

# ╔═╡ 86397c61-9e6f-4c59-9b64-e06857f11e2c
md"""
Then we define the numerical problem and solve.
"""

# ╔═╡ 2bd77943-c123-4a44-8927-1282f28221ee
begin
	q2 = 1.0
	m2 = 1.0
	mu2 = 1.0 # Constant
	I2 = 1.0
	R2 = 0.25 # Radius of the loop
	param = (q2, m2, R2, I2, mu2) # charge, mass, radius
	prob = ODEProblem(magneticsystem!, u0, tspan, param)
end;

# ╔═╡ b73b3d6e-1efd-4413-8d6e-3faf5b86fa20
sol = solve(prob)

# ╔═╡ b53e9522-016b-4822-af9f-1e66a9ac83d9
md"""
### Visualization
"""

# ╔═╡ 6902308d-1458-415f-bab0-59d8171b8de1
begin
	# Size of numerical window in terms of circle size
	xmaxw = 3.5
	pointsw = 8
	...
	# Generates ranges for xs, ys and zs
	xsw = xmaxw*(2/pointsw)*collect(range(-pointsw/2,length=pointsw,stop=pointsw/2-1))
	ysw = xmaxw*(2/pointsw)*collect(range(-pointsw/2,length=pointsw,stop=pointsw/2-1))
	zsw = xmaxw*(2/pointsw)*collect(range(-pointsw/2,length=pointsw,stop=pointsw/2-1))
	
	# Coordinates ... is it correct?
	mxw, nyw, pzw = length(xsw), length(ysw), length(zsw)
	Xsw = repeat(xsw, 1, nyw, pzw)
	Ysw = repeat(ysw', mxw, 1, pzw)
	Zsw = repeat(reshape(zsw, 1, 1, pzw), mxw, nyw ,1);
	
	# The parameters are defined before :)
    wBx = wireBx.(Xsw,Ysw,Zsw,mu2,I2,R2)
    wBy = wireBy.(Xsw,Ysw,Zsw,mu2,I2,R2)
    wBz = 0.0 .* Zsw
end;

# ╔═╡ 9dcbd2c8-6f7a-44d2-9d62-0efeb8323be9
begin
	Msol = convert(Array,sol)
	lli = 2*maximum(abs,(Msol[[1,3,5],:]))

	# Field
	fac2 = 50;
	p2 = plot3d(bg=RGB(1,1,1))
	p2 = carcaj(Xsw, Ysw, Zsw, fac2*wBx, fac2*wBy, fac2*wBz; samppx=2^3, samppy=2^3, samppz=2^2)
	
	# Trajectory!
	p2 = plot!(sol, idxs=(1,3,5), xlims=[-div(lli,2),div(lli,2)], ylims=[-div(lli,2),div(lli,2)], zlims=[-div(lli,2),div(lli,2)], color=:blue, linewidth=2.0, linestyle = :dash)
	p2 = scatter3d!((Msol[1,1], Msol[3,1], Msol[5,1]))
	p2 = scatter3d!((Msol[1,end], Msol[3,end], Msol[5,end]))	# Draw axis to overcome weird scaling behaviour
	winsizew = 0.8*maximum([maximum(abs,Xsw),maximum(abs,Ysw),maximum(abs,Zsw)])
	ejes(winsizew; cl=:black)
end;

# ╔═╡ 6e5bde98-07f1-4791-9ca2-08ba6f75ca8d
md"""
anglec1 $(@bind ancA2 Slider(0.0:1.0:360, default=45.0, show_value=true))
anglec2 $(@bind ancP2 Slider(0.0:1.0:360, default=45.0, show_value=true))
"""

# ╔═╡ 484009d4-3c7c-4553-a584-625f280a38e6
plot(p2, proj_type=:persp, leg=:false, camera=(ancA2,ancP2), size=(1920,1920))
...
julia_version = "1.9.0"
manifest_format = "2.0"
project_hash = "fcb4616c15bd30c9928be9a32a854993bbee4310"
...
git-tree-sha1 = "ed01a68c22907665e2f5a65ccc58668b6a764536"
